...
document.addEventListener('DOMContentLoaded', () => {
    ...
    switchButton.addEventListener('click', () => {
        ...
        recaptchaGroup.style.display = isHidden ? 'block' : 'none';
    ...
    authForm.addEventListener('submit', async (e) => {
        ...
        if (isSignUp) {
            ...
            // Send reCAPTCHA response to server for validation
            const recaptchaValidation = await validateRecaptcha(recaptchaResponse);
            if (!recaptchaValidation.success) {
                errorMessage.textContent = 'Weryfikacja reCAPTCHA nie powiodła się.';
                errorMessage.style.display = 'block';
                return;
            }
        }
